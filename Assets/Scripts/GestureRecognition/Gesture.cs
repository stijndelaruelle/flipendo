...
public class Gesture
{
    ...
    public Result Recognize(GestureLibrary gestureLibrary, bool fast = false)
	{
		...
		else
		{
            ...
            for (int i = 0; i < library.Count; i++)
            {
                ...
                if (fast) d = GetOptimalCosineDistance(library[i].Vector, this.Vector);
                else      d = GetDistanceAtBestAngle(library[i], -this.ANGLE_RANGE, +this.ANGLE_RANGE, this.ANGLE_PRECISION);
	...
	{
        if (points1.Count == 0 || points2.Count == 0)   return 0.0f;
        if (points1.Count != points2.Count)             return 0.0f;
